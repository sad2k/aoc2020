...
getComplexParser m r = if '|' `elem` r
    then
        choice $ map (\x -> try $ getComplexParser m x) (splitOn " | " r)
    else do
...
getParser rules n = case (Map.lookup n rules) of
...
isValid s parser = case (parse parser "" s) of
                  Left err -> False
                  Right e -> length e == length s
